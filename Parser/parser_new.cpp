...
class Parser
{
	...
	private:
		...
		int current = 0; //index of the character currently being evaluated in source
		int line = 1; //the line of the character that current points at
...
void Parser::scan_token()
{
	...
	switch (c)
	{
		...
		default :
		{
			...
			{
				Error{this->line, this->get_current_line(), std::format("unexpected character: {}", c)}.report();
				this->was_error = true;
				break;
			}
		}
